...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Transport3_1[] arrTransport = new Transport3_1[arr.length];
        ...
        for (int i = 0; i < arrTransport.length; i++) {
            if (arr[i].contains("C100")) {
                ...
                arrTransport[i] = new Light3_1(100, Integer.parseInt(carParams[0]), Integer.parseInt(carParams[1]));
                fuelAmountLight += arrTransport[i].calcFuel();
                expensesLight += arrTransport[i].calcExpenses();
            }
            if (arr[i].contains("C200")) {
                String[] carParams = arr[i].substring(5).split("-");
                arrTransport[i] = new Cargo3_1(200, Integer.parseInt(carParams[0]), Integer.parseInt(carParams[1]), Integer.parseInt(carParams[2]));
                fuelAmountCargo += arrTransport[i].calcFuel();
                expensesCargo += arrTransport[i].calcExpenses();
            }
            if (arr[i].contains("C300")) {
                String[] carParams = arr[i].substring(5).split("-");
                arrTransport[i] = new Passenger3_1(300, Integer.parseInt(carParams[0]), Integer.parseInt(carParams[1]), Integer.parseInt(carParams[2]));
                fuelAmountPassenger += arrTransport[i].calcFuel();
                expensesPassenger += arrTransport[i].calcExpenses();
            }
            if (arr[i].contains("C400")) {
                String[] carParams = arr[i].substring(5).split("-");
                arrTransport[i] = new Heavy3_1(400, Integer.parseInt(carParams[0]), Integer.parseInt(carParams[1]), Integer.parseInt(carParams[2]));
    ...
    public static void showSortedTransport(Transport3_1[] arrTransport, int type, String field) {
        int countType = 0;
        //определяем количество транспортов заданного типа
        for (int i = 0; i < arrTransport.length; i++) {
            if (arrTransport[i].getCodeCar() == type) {
                countType++;
            }
        }
        //создаем массив транспортов необходимого размера
        Transport3_1[] thisTransport = new Transport3_1[countType];
        ...
        for (int i = 0; i < thisTransport.length; i++) {
            for (int j = 0; j < thisTransport.length - 1; j++) {
                if (thisTransport[j].getMileage() > thisTransport[j + 1].getMileage() && (field == "mileage" || type == 100)) {
                    Transport3_1 tempTransport = thisTransport[j];
                    thisTransport[j] = thisTransport[j + 1];
                    thisTransport[j + 1] = tempTransport;
                }
                else if (thisTransport[j].getParam() > thisTransport[j + 1].getParam() && field == "param") {
                    Transport3_1 tempTransport = thisTransport[j];
                    thisTransport[j] = thisTransport[j + 1];
                    thisTransport[j + 1] = tempTransport;
                }
        ...
        for (int i = 0; i < thisTransport.length; i++) {
            System.out.println("Тип: " + thisTransport[i].getCodeCar() + ", номер: " + thisTransport[i].getGosNumber() +
                    ", пробег: " + thisTransport[i].getMileage() + ", доп. параметр: " + thisTransport[i].getParam());
        }
    }
}
